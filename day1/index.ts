const calibrationString = `nqninenmvnpsz874
...
// const calibrationString = `two1nine
// eightwothree
// abcone2threexyz
// xtwone3four
// 4nineeightseven2
...
// const calibrationString = `eightwothree`;

// const calibrationString = `8twofpmpxkvvdnpdnlpkhseven4ncgkb
// six8shdkdcdgseven8xczqrnnmthreecckfive
// qlcnz54dd75nine7jfnlfgz
// 7vrdhggdkqbnltlgpkkvsdxn2mfpghkntzrhtjgtxr
// cdhmktwo6kjqbprvfour8
...
const transformLine = (line: string): string => {
   ...
   for (let i = 3; i <= line.length; i++) {
      ...
      if (NUMBER_WORDS_MAP[subString]) {
         return `${NUMBER_WORDS_MAP[subString]}${transformLine(line.slice(1))}`
      }
   }

   return line[0] + transformLine(line.slice(1))
}

const getNumber = (char: string): string | void => {
...
const parseLine = (line: string): number => {
   ...
   for (const char of transformedLine) {
      if (getNumber(char)) {
         numbers.push(getNumber(char)!)
      }
   }
   ...
   let ret
   if (numbers.length === 0) {
      throw new Error("Not enough numbers in line")
   } else if (numbers.length === 1) {
      ret = Number(`${numbers[0]}${numbers[0]}`)
   } else {
      ret = Number(`${numbers[0]}${numbers[numbers.length - 1]}`)
   }
   // console.log({ numbers, ret })
   return ret
}

// Run
(() => {
   const lines: string[] = calibrationString.split("\n");
   ...
   console.log(result)
   console.log(result === 281 ? 'RIGHT' : 'WRONG')
})();
