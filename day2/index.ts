const input = `Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
...
// First star
const possibilityCriteria = {
   red: 12,
   green: 13,
   blue: 14,
};

const getMaxPerLine = (line: string): { red: number, green: number, blue: number } => {

   return {
      red: Math.max(...line.match(/[^\s]+ (?=red)/g).map(Number)),
      blue: Math.max(...line.match(/[^\s]+ (?=blue)/g).map(Number)),
      green: Math.max(...line.match(/[^\s]+ (?=green)/g).map(Number)),
   }
}
...
//    const lineMaxes = lines.map((line, index) => {
//       return {
//          id: index + 1,
//          ...getMaxPerLine(line)
//       }
...
//    const possibleGames = lineMaxes.filter(maxes => {
//       return maxes.green <= possibilityCriteria.green
//          && maxes.blue <= possibilityCriteria.blue
//          && maxes.red <= possibilityCriteria.red
//    })
//    const finalSum = possibleGames.reduce((acc, curr) => {
//       return acc + curr.id
//    }, 0)
...
const run = () => {
   ...
   const linePowers = lineMaxes.map(maxes => {
      return maxes.blue * maxes.red * maxes.green
   })

   const sums = linePowers.reduce((acc, curr) => {
      return acc + curr
   })
   console.log({ sums })
}
