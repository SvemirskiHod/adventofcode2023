...
const isSymbol = (char) => {
   return Boolean(char && char !== '.') && !isNumber(char)
}

const parseNumbers = (line: string[]): { start: number, end: number }[] => {
   const numbers = []
   line.forEach((char, index) => {
      if (isNumber(char)) {
         const isLastNumber = isNumber(line[index - 1])
         const isNextNumber = isNumber(line[index + 1])

         if (!isLastNumber) {
            numbers.push({ start: index })
         }
         if (isLastNumber && !isNextNumber) {
            numbers[numbers.length - 1].end = index
         }
      }
   })
